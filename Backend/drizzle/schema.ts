...
export const user = pgTable(
  ...
  (table) => {
    return {
      rollnoKey: uniqueIndex("user_rollno_key").using(
        "btree",
        table.rollno.asc().nullsLast()
      ),
...
export const submissionRelations = relations(submission, ({ one, many }) => ({
  ...
  testManager: one(testManager, {
    fields: [submission.tmid],
    references: [testManager.tmid],
  }),
}));
...
export const questionBankRelations = relations(
  questionBank,
  ({ one, many }) => ({
    options: many(option),
    test: one(test, {
